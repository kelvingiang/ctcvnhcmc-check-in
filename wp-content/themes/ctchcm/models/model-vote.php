...
class Admin_Vote_Model extends WP_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        $sorttable = $this->get_sortable_columns(); /* CAC COT DC SAP XEP TANG HOAC GIAM DA */
    ...
    public function get_sortable_columns()
    {
        return array(
            'kid' => array('kid', true),
            'vote_total' => array('vote_total', true),
        );
    }
    ...
    {
        if (!empty($item['img'])) {
            echo '<img style="width:50px" src=" ' . get_vote_img($item["img"]) . ' " />';
        }
    }

    public function column_kid($item)
    {
        echo "<lable style=' letter-spacing: 5px;;'>" . kid_name($item['kid']) . "</lable>";
    }

    public function column_vote_total($item)
    {
        echo '<lable>' . $item["vote_total"] . '</lable>';
    }
    ...
    private function del_img($id)
    {
        ...
        /* XOA HINH DAI DIEN */
        if (is_file(HCM_DIR_IMAGES . 'vote/' . $row['img'])) {
            unlink(HCM_DIR_IMAGES . 'vote/' . $row['img']);
        }
    }
    ...
    public function saveItem($arrData = array(), $imgFile, $option = array())
    {
        ...
        $data = array(
            ...
            'number' => trim($arrData['txt_number']),
            'img' => $imgFile,
            'kid' => $arrData['sel_kid'],
            'vote_total' => $arrData['txt_vote'],
        );
    ...
    public function resetVoteToZero()
    {
        global $wpdb;
        $table = $wpdb->prefix . $this->table;
        $sql = "UPDATE $table SET `vote_total` =  '0'   WHERE 1 =1 ";
        $wpdb->query($sql);
    }

    public function getVoteResult($kid)
    {
        global $wpdb;
    ...
    {
        $data = $this->getVoteResult($kid);
        export_excel_vote($data);
    }
}
